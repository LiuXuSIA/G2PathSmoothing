...
def reeds_shepp_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.01):
...
def generate_final_path(rx, ry, ds, curvature):
    ...
    for i in range(len(road_points)-1):
        px, py, pyaw, mode, course_lengths = \
            reeds_shepp_path_planning(road_points[i][0], road_points[i][1], road_points[i][2],
            road_points[i+1][0], road_points[i+1][1], road_points[i+1][2], curvature, ds)
        
        trajs.extend((np.c_[px, py]).tolist())

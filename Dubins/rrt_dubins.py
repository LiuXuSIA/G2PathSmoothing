...
class RRTDubins(RRT):
    ...
    def __init__(self, start, goal, obstacle_list, robot_radius, rand_area,
                 goal_sample_rate=10,
                 max_iter=200,
                 ):
        """
    ...
    def generate_final_course(self, goal_index):
        print("final")
        points = [[self.end.x, self.end.y, self.end.yaw]]
        ...
        while node.parent:
            ...
            points.append([node.x, node.y, node.yaw])
            node = node.parent
        points.append([self.start.x, self.start.y, self.start.yaw])
...
def main():
    ...
    rrt_dubins = RRTDubins(start, goal, obstacleList, 0.5, [-2.0, 15.0])
    path, points = rrt_dubins.planning(animation=show_animation)

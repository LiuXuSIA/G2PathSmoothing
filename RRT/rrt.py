...
def consistency_error(trajectory, path):
    ...
    tree = KDTree(path)
    ind = tree.query(trajectory, k=1, return_distance=False)
    nearest_points = path[ind[:,0]]
    average_d = np.mean(np.sqrt((trajectory[:,0]-nearest_points[:, 0])**2 + (trajectory[:,1]-nearest_points[:, 1])**2))
    maximum_d = np.max(np.sqrt((trajectory[:,0]-nearest_points[:, 0])**2 + (trajectory[:,1]-nearest_points[:, 1])**2))
...
def main(gx=28, gy=28):
    ...
    log_file = rootPath + 'experiments\\'+'sce0_'+ now+'_rr'+str(robotRadius)+'_ed'+str(expandDis)+'_pr'+str(pathResolution)+'.txt'
    ...
    while planning_time < 10:
        try:
            ...
            path_smoothed_cubicBezie = cubic_bezier.pathsmoothbezier(np.array(points)[:,0],np.array(points)[:,1], 0.01, 2)[:,0:2]
            ...
            path_smoothed_dubins = dubins.generate_path(np.array(points)[:,0],np.array(points)[:,1], 0.01, 2)
            ...
            path_smoothed_reeds_shepp = reeds_shepp.generate_final_path(np.array(points)[:,0],np.array(points)[:,1], 0.01, 2)
